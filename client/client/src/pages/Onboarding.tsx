...
const Onboarding = () => {
  ...
  const handleSubmit = async () => {
    ...
    const submissionPayload = {
      ...
      lastPeriodStart: formData.gender === "female" && formData.useMenstrualTracker ? formData.lastPeriodStart : "",
      avgCycleLength: formData.gender === "female" && formData.useMenstrualTracker ? formData.avgCycleLength : 0,
      avgBleedingDays: formData.gender === "female" && formData.useMenstrualTracker ? formData.avgBleedingDays : 0,
    };
    ...
    try {
      ...
      console.error(err);
      alert("Network runtime initialization failed to commit transaction request.");
    }
  };
        ...
        <form onSubmit={handleNext} className="space-y-6">
          ...
          {step === 1 && (
            <div className="space-y-4 animate-fadeIn">
              ...
              <p className="text-sm text-slate-400">Please select your gender to help personalize your experience.</p>
          ...
          {step === 2 && (
            <div className="space-y-4">
              <h2 className="text-2xl font-bold tracking-tight text-white">Body Metrics</h2>
              <p className="text-sm text-slate-400">We use these numbers to accurately calculate baseline profiles.</p>
              <div className="pt-2 space-y-5">
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    onChange={(e) => updateFields({ height: parseInt(e.target.value) || 0 })}
          ...
          {/* STEP 3: CYCLE TRACKING QUESTION */}
          {step === 3 && formData.gender === "female" && (
            <div className="space-y-4">
              <h2 className="text-2xl font-bold tracking-tight text-white">Cycle Tracking</h2>
              <p className="text-sm text-slate-400">Would you like to integrate a menstrual cycle tracker into your dashboard?</p>
              <div className="grid grid-cols-2 gap-4 pt-2">
                <button
                  type="button"
          ...
          {/* STEP 4: TRACKING PROFILE PARAMETERS */}
          {step === 4 && formData.gender === "female" && formData.useMenstrualTracker && (
            <div className="space-y-4 animate-fadeIn">
              <h2 className="text-2xl font-bold tracking-tight text-white">Cycle Details</h2>
              <div className="space-y-4">
                <div>
                  <label className="block mb-1 text-sm text-slate-400">Last Period Start Date</label>
                  <input
                    type="date"
                ...
                <div>
                  <label className="block mb-1 text-sm text-slate-400">Average Cycle Length (Days)</label>
                  <input
                    type="number"
                    value={formData.avgCycleLength}
                    onChange={(e) => updateFields({ avgCycleLength: Number(e.target.value) })}
                    className="w-full p-3 text-white border rounded-xl bg-slate-800 border-slate-700"
                  />
                </div>
                <div>
                  <label className="block mb-1 text-sm text-slate-400">Avg Bleeding Days</label>
                  <input
                    type="number"
                    value={formData.avgBleedingDays}
                    onChange={(e) => updateFields({ avgBleedingDays: Number(e.target.value) })}
                    className="w-full p-3 text-white border rounded-xl bg-slate-800 border-slate-700"
                  />
                </div>
              </div>
          ...
          <div className="flex items-center justify-between gap-4 pt-4 border-t border-slate-800">
            ...
            >
              {step === 4 || (step === 3 && !formData.useMenstrualTracker) || (step === 2 && formData.gender !== "female")
                ? "Complete Setup"
                : "Next Step"}
            </button>
          </div>
